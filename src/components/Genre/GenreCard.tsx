...
const GENRE_ICONS: Record<GenreId, ReactElement> = {
  "cosmic-horror": (
...
export function GenreCard({ genreId, isSelected, onClick }: GenreCardProps) {
  ...
  return (
    <button
      ...
      className={`
        ...
        ${
          isSelected
            ? "bg-[#d4a574]/10 border-[#d4a574] scale-[1.02]"
            : "bg-[#1a1816] border-[#3d3a38] hover:border-[#d4a574]/50 hover:bg-[#1a1816]/80"
        }
      `}
    >
      {/* Selection indicator */}
      ...
      <div
        className={`
          ...
          ${isSelected ? "text-[#d4a574]" : "text-[#78716c]"}
        `}
      >
        {GENRE_ICONS[genreId]}
      </div>
      ...
      <h3
        className={`
          ...
          ${isSelected ? "text-[#f5f5f5]" : "text-[#a8a29e]"}
        `}
      >
        {config.name}
      </h3>
      ...
      {/* Tagline */}
      <p className="text-sm text-[#78716c] italic">{config.tagline}</p>
    </button>
  );
}
